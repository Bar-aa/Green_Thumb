...
const createPartnership = async (name, description, contactInfo) => {
    return new Promise((resolve, reject) => {
        const query = 'INSERT INTO localpartnerships (name, description, contact_info) VALUES (?, ?, ?)';
        db.query(query, [name, description, contactInfo], (error, result) => {
            if (error) {
                reject(error);
            } else {
...
const updatePartnership = async (partnershipId, name, description, contactInfo) => {
    return new Promise((resolve, reject) => {
        const query = 'UPDATE localpartnerships SET name = ?, description = ?, contact_info = ? WHERE partnership_id = ?';
        db.query(query, [name, description, contactInfo, partnershipId], (error, result) => {
            if (error) {
                reject(error);
            } else {
